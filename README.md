...
**Планы запросов будут выведены вместе с самими запросами при запуске скрипта. Но также их можно увидеть в файле [`QUERY_PLANS.pdf`](QUERY_PLANS.pdf) (препросмотр недоступен)**. Планы в файле сделаны при разогретом кэше, поэтому время выполнения запроса в файле будет отличасться от того, что будет при первом запуске.
...
Например, если мы анализируем только те посты, которые имеют тэг `postgresql`, то можно создать и поддерживать соответсвующие материализованное представление.
