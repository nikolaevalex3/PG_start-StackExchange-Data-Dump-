...
По аналогии с запросом Q1 была предпринята попытка фильтровать тэги из таблицы Posts, но в этот раз такой подход ожидаемо оказался проигрышным, так как не было такого большого количества JOIN'ов, которых надо было избежать.
...
Например, если мы анализируем только те посты, которые имеют тэг `postgresql`, то можно создать и поддерживать соответсвующее материализованное представление.
