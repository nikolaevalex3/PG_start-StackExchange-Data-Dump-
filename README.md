...
**Результаты без дополнительной оптимизации**:
- `Q1_with_PostTags.sql` — немного быстрее.
...
После создания GIN-индекса с `pg_trgm` повторное тестирование показало:
> **Запрос `Q1_with_LIKE.sql` стал выполняться быстрее на ~50 мс** по сравнению с вариантом с многочисленными JOIN'ами.
