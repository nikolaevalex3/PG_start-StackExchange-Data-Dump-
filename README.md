# Миграция данных StackExchange в PostgreSQL

## Задача:
Перенос схемы и данных StackExchange (dba.stackexchange.org и dba.meta.stackexchange.org) из архивов дампов в PostgreSQL с последующим восстановлением связей между таблицами, выполнением запросов и оптимизацией.
...
- `meta` — содержит данные с сайта `dba.meta.stackexchange.com` и уже находится в нужном месте.
...
1. **`fields.py`**  
   Проверяет, какие поля реально присутствуют в XML-файлах дампа, чтобы при создании схемы учесть только нужные столбцы. Выводит список актуальных полей по каждой таблице. Не забудьте установить зависимости из файла requirements.txt
...
**Результаты без дополнительной оптимизации с горячим кэшем**: `Q1_with_PostTags.sql` — немного быстрее.
...
**Планы запросов будут выведены вместе с самими запросами при запуске скрипта. Но также их можно увидеть в файле [`QUERY_PLANS.pdf`](QUERY_PLANS.pdf) (предпросмотр недоступен)**. Планы в файле сделаны при разогретом кэше, поэтому время выполнения запроса в файле будет отличаться от того, что будет при первом запуске.

### Визуализация
Я использовал Metabase. Запрос пришлось немного изменить, чтобы показать визуализацию соответсвующую задаче. Визуализация ограничена пятнадцатью самыми популярными парами.
1. Количество пар тегов

![Количество пар тегов](images/Tag_pair_and_count.png)

2. Среднее время ответа для вопросов с парой тегов

![Среднее время ответа для вопросов с парой тегов](images/tag_pair_and_time.png)

3. Средняя репутация пользователей, ответивших на вопросы с парой тегов

![Средняя репутация пользователей, ответивших на вопросы с парой тегов](images/tag_pair_and_reputation.png)

4. Связи между тегами по количеству совместных использований

![Связи между тегами по количеству совместных использований](images/tag_pair_connection_count.png)
...
По аналогии с запросом Q1 была предпринята попытка фильтровать теги из таблицы Posts, но в этот раз такой подход ожидаемо оказался проигрышным, так как не было такого большого количества JOIN'ов, которых надо было избежать.

### Визуализация
1. Ответы с отрицательм рейтингом

![Ответы с отрицательм рейтингом](images/joker_and_score.png)

## Что можно улучшить?

### Секционирование
Можно рассмотреть секционирование двух самых больших таблиц - `Posts` и `PostHistory`.

В таблице `Posts` можно создать 2 слоя секционирования:
1. Секционирование по `creationdate` (месяца)
2. Секционирование по `posttypeid` (Вопросы = 1 | Ответы = 2 | Все остальные типы)

Таблицу `PostHistory` на мой взгляд достаточно секционировать по дате.

В таком случае доступ к этим большим таблицам будет происходить быстрее, когда фильтрация в запросе будет совпадать с принципом секционирования.

### MATERIALIZED
Если мы можем себе позволить создавать MATERIALIZED представления, то имеет смысл рассмотреть этот вариант, так как это иногда может значительно ускорить запросы. 
Например, если мы анализируем только те посты, которые имеют тег `postgresql`, то можно создать и поддерживать соответсвующее материализованное представление.
